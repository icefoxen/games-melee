(* clientrealms.ml
   ...
   XXX: We do real calculation now.  Next up: client input.
...
(* The handshake SHOULD contain all the data the client needs, right?
   Right?
*)
class realm serverhandshake serveriaddr ic oc =
  (*  let cfg = getConfig cfgfile in *)
  let listenport = (serverhandshake.sh_dataport + 2 +
		    (int_of_char serverhandshake.sh_clientid * 2)) in
  let sendport = (serverhandshake.sh_dataport + 1 +
		  (int_of_char serverhandshake.sh_clientid * 2)) in
...
(*  val selfsockaddr = ADDR_INET( getMyIP (), selfdataport ) *)
  ...
  val mutable sendInterval = 20

  (* XXX: Background should be from config file *)
  val mutable background = getImg "bigbackground4.png"
...
*)
  ...
  (* We do bandwidth throttling here too, but it's a lot simpler since
     ...
     XXX: We may want to send events more rapidly, or whenever the keystate
  ...
  *)
  method doServerInput =
	...
	Playere( e ) -> 
	  if e.pe_time > player#getLastUpdate then (
	    player#setLastUpdate e.pe_time;
	    player#moveTo (float_of_int e.pe_x) (float_of_int e.pe_y);
	    player#setFacing (float_of_int e.pe_facing);
	  )
      | Objecte( e ) -> 
