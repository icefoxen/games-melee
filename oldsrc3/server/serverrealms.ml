...
object (self)
  ...
  val netq = new Netq.netq dataport
...
*)
  ...
  *)
  ...
  method doClientOutput =
	...
	print_endline "Player data sent";
	...
	*)
	...
	print_endline "Object data sent";
  ...
  *)
  ...
  method doMainloop =
    ...
    let mapobjs = (new Maploader.maploader 
		     (!Resources.datadir ^ map))#getObjects in
      self#addObjects mapobjs;


      (* Create object index.  Why oh why do I avoid initializers so much,
	 anyway? 
	 ...RAR.  The typeindex has to be init'd in the main function,
	 so's we can use it to send the handshake to the client.
	 Or not, as the case may be.
      *)
      let namearr = Array.of_list 
		      (List.map (fun x -> x#getConfig) mapobjs) in
