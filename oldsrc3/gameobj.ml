...
q   index may not be init'ed when we create a gameobject!
   Hey, I just invented late binding!  *)
  method getTypeIndex =
    print_string "getTypeIndex called with ";
    print_int typeIndex;
    print_newline ();
    if typeIndex < 0 then (
      print_string "Initing typeindex, which is... ";
      ...
      print_string "Typeindex already exists: ";
      print_int typeIndex;
      print_newline ();
      typeIndex
    )
  ...
  *)
  ...
  (* XXX: This is somewhat horribly wrong...  Getting less so tho.
     We just have to make fucking energy work...  And handling, that is.
  *)
  ...
  *)
  ...
  method toPacket time =
    print_endline ("toPacket called for " ^ cfgfile);
    let p = {
      Net.oe_id = id;
