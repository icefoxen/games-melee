...
let loadImg name =
  try
    let n = Sdlvideo.display_format ~alpha: true 
	      (Sdlloader.load_image (!datadir ^ "images/" ^ name)) in
...
let loadSound name =
  try
    let n = Sdlmixer.loadWAV (!datadir ^ "audio/" ^ name) in
...
let loadConfig name =
  let n = new Config.config (!datadir ^ name) in
    Hashtbl.add configtbl name n
;;


let loadFont name =
  let n = Sdlttf.open_font (!datadir ^ "fonts/" ^ name) 16 in
