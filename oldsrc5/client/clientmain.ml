...
let mainloop scr handshake serveraddr ic oc =
  try
    let r = new Clientrealms.realm handshake serveraddr ic oc in
      r#doMainloop scr;
  with
	...
	and oc = out_channel_of_descr socket in
	  ...
	  let serverhs = getHandshakeFromServer ic in
	    ...
	    let screen = set_video_mode ~w: !screenx ~h: !screeny ~bpp: 16 
		 ...
		 However, it is no longer necessary 'cause we now say
		 that 
	      *)
	      (*
		let dumpConfig c =
		...
		Array.iter dumpConfig serverhs.sh_objindex;
	      *)
	      (* Mainloop *)
	      mainloop screen serverhs serveraddr ic oc; 
