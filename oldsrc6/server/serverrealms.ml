...
*)
  ...
  *)
  method doClientInput dt =
	...
	if im#isKeyPressed Input.firemask && ship#isAlive then
	  self#addObjects (ship#makeShot : Weapon.shot list :> Gameobj.gameobj list);
	if im#isKeyPressed Input.specialmask && ship#isAlive then
	  ();
  ...
  *)
  method doClientOutput =
    ...
    if isnow > nextSend then (
	 ...
	 what.  If we're having less than one game loop per 
	 throttleinterval, we have bigger problems than some extra lag.  *)
  ...
  method doCalc dt =
    ...
    let doReverseAccel o =
      if o#getX > dimX then
	o#acceldm 270. ((o#getX -. dimX) *. t /. wallToughness)
      else if o#getX < -. dimX then
	o#acceldm 90. ((-. dimX -. o#getX) *. t /. wallToughness);
      if o#getY > dimY then
	o#acceldm 180. ((o#getY -. dimY) *. t /. wallToughness)
      else if o#getY < -. dimY then
	o#acceldm 0. ((-. dimY -. o#getY) *. t /. wallToughness);
    in
  ...
  *)
  method doDeath =
    (* First we handle dead players...  *)
    Array.iter (fun x -> if not x#isAlive then 
		  x#getShipobj#setTangible false) players;
