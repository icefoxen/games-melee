...
(* Okay, this contains all the info necessary for a player.
   ...
   XXX: We may end up having to put in another netq for TCP communication.
*)
...
object (self)
  ...
  val mutable lastClientUpdate = 0
  ...
  *)
  ...
  method getLastClientUpdateTime = lastClientUpdate
  method setLastClientUpdateTime x = 
    lastClientUpdate <- (max x lastClientUpdate)
  ...
  method getClientData =
    let input = (netq#getObjects : Net.playerInput list) in
      (*      List.iter 
  ...
  *)
  ...
  (* The isAlive check is a bit of a kludge 
     This entire thing is screwy.  Shall I do the ghost-ship hack instead?
     I kinda think so...
  *)
  method cleanUpNet =
    try
    if netq#isAlive then (
      netq#kill;
    )
    with
	x -> print_endline "Foo!"; raise x;
